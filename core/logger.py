...
import logging, inspect, os, sys, warnings
...
class FilterDuplicateWarning(logging.Filter):
    ...
    def __init__(self, name: str = 'VQA_Toolkit'):
        super().__init__(name)
        self.seen: set = set()
...
class VQAFormatter(logging.Formatter):
    """Colorful format for VQAFormatter. If the log level is error, the logger will
...
class VQALogger(Logger, ManagerMixin): 
    """Formatted logger used to record messages.

    ``VQALogger`` can create formatted logger to log message with different 
    log levels.
    ``VQALogger`` has the following features:

    - Distributed log storage, ``VQALogger can choose whether to save log of
    ...
    Note:
        - The `name` of logger and the ``instance_name`` of ``VQALogger`` could
          be different. We can only get ``VQALogger`` instance by
          ``VQALogger.get_instance`` but not ``logging.getLogger``. This feature
          ensures ``VQALogger`` will not be incluenced by third-party logging
          config.
        - Different from ``logging.Logger``, ``VQALogger`` will not log warning
          or error message without ``Handler``.

    Examples:
        >>> logger = VQALogger.get_instance(name='VQALogger',
                                            logger_name='Logger')
        ...
        >>> assert logger.instance_name = 'VQALogger'
        >>> assert id(logger) != id(logging.getLogger('Logger'))
        >>> # Get logger that do not store logs.
        >>> logger1 = VQALogger.get_instance('logger1')
        >>> # Get logger only save rank0 logs.
        >>> logger2 = VQALogger.get_instance('logger2', log_file='out.log')
        >>> # Get logger only save multiple ranks logs.
        >>> logger3 = VQALogger.get_instance('logger3', log_file='out.log',
        >>>                                 distributed=True)
    ...
    Args:
        ...
        logger_name(str): ``name`` attribute of ``logging.Logger`` instance.
            If `logger_name` is not defined, defaults to 'VQA_Toolkit'.
        log_file(str, optional): The log filename. If specified, a 
            ``logging.FileHandler`` will be added to logger. Defaults to None.
    ...
    def __init__(self,
        ...
        stream_handler.setFormatter(
            VQAFormatter(color=True, datefmt='%m/%d %H:%M:%S'))
        ...
        if log_file is not None:
            ...
            if global_rank == 0 or is_distributed:
                ...
                file_handler.setFormatter(
                    VQAFormatter(color=False, datefmt='%Y/%m/%d %H:%M:%S'))
    ...
    @classmethod
    def get_current_instance(cls) -> 'VQALogger':
        """Get latest created ``VQALogger`` instance.

        :obj:`VQALogger` can call :meth:`get_current_instance` before any 
        instance has been created, and return a logger with the instance name 
        "VQA_Toolkit".

        Returns:
            VQALogger: Configured logger instance
        """
        if not cls._instance_dict:
            cls.get_instance('VQA_Toolkit')
        return super().get_current_instance()

    def callHandlers(self, record: LogRecord) -> None:
        """Pass a record to all relavant handlers.
        ...
        handler is found, the record will not be output.
    ...
    def setLevel(self, level):
        ...
        If ``logging.Logger.setLevel`` is called, all ``logging.Logger`` 
        instance managed by ``logging.Manager`` will clear the cache. Since
        ``VQALogger`` is not managed by ``logging.Manager`` anymore, 
        ``VQALogger`` should override this method to clear caches of all 
        ``VQALogger`` instances which is managed by :obj:`ManagerMixin`.
        ...
        for logger in VQALogger._instance_dict.values():
            logger._cache.clear()
        _release_lock()
...
def print_log(msg, 
    ...
    Args:
        ...
        logger (Optional[Union[Logger, str]], optional): If the type of logger is 
        ``logging.Logger``, we directly use logger to log messages.
            Some special loggers are:

            - "silent": No messgae will be printed.
            - "current": Use latest created logger to log messages.
    ...
    if logger is None:
        ...
        logger_instance = VQALogger.get_current_instance()
        ...
        # a `ValueError`.
        if VQALogger.check_instance_created(logger):
            logger_instance = VQALogger.get_instance(logger)
            logger_instance.log(level, msg)
        else:
        ...
        raise TypeError(
            ...
            f'"current", or "silent", but got {type(logger)}'
        )
